...
setwd("C:/Users/Ike/Desktop/school/College/Statistics/Stats 480/Final Project")
states_location_for_texts = read.csv("statelatlong.csv")

#states_location_for_texts = read.csv("https://www.kaggle.com/washimahmed/usa-latlong-for-state-abbreviations#statelatlong.csv")
...
ggplot(Republican_State_Score_map, aes(x = long, y = lat)) + 
  ...
  scale_fill_gradient(low = "red", high = "white", na.value = NA) + #might want to change the lower scores --> darker color
...
annotate(geom = "text", x = -75, y = 44, label = "NY", size = 5, fontface = "bold") +
  ...
  annotate(geom = "text", x = -72.5, y = 38.5, label = "DE", size = 5, fontface = "bold") +
  annotate(geom = "text", x = -74, y = 37.7, label = "MD", size = 5, fontface = "bold") +
  annotate(geom = "text", x = -74.5, y = 37, label = "DC", size = 5, fontface = "bold") +
